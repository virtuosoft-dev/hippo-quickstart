...
<?php
    ...
    // Check for download file url
    if ( ! isset( $_GET['url'] ) ) {
        header( 'Location: ?quickstart=main' );
        exit;
    }

    // Start the download asynchronously if not already started (PID unknown)
    global $hcpp;
    $url = $_GET['url'];

    // Sanitize URL, prevent folder traversing, and set job data
    $url = filter_var( $url, FILTER_SANITIZE_URL );
    $url = str_replace( '..', '', $url );
    $file = "/tmp/devstia_$job_id-" . basename( $url );
    if ( !file_exists( $file ) ) {
        $hcpp->quickstart->download_file( $job_id, $url );
    }
...
<div class="body-reset container">
    <div class="quickstart qs_create_options">
        ...
        <legend id="status">Please wait. Downloading blueprint files.</legend>
        <div id="options"></div>
    </div>
</div>
<script>
(function($){
    $(function() {
        ...
        // Check the download status every 3 seconds
        var download_int = setInterval( () => {
            $.ajax({
                url: '../../pluginable.php?load=quickstart&action=download_status&job_id=<?php echo $job_id; ?>',
                type: 'GET',
                success: function( data ) {
                    ...
                    if ( data.status == 'finished' ) {
                        $('#status').html(data.message || 'Download complete.');
                        clearInterval( download_int );
                    } else {
                        console.log(data);
